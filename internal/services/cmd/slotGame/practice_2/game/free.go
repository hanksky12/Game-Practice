...
func (f *Free) Run(settingsTables *settings.WayTables) {
	log.Info("----------------------FG start----------------------")
	...
	base := table.Base{Board: board, MinimumCount: f.GameParam.PayOutMinimumCount.Fg}
	score := base.CalculateWinScore(f.GameParam.SingleBet, settingsTables.Pay)
