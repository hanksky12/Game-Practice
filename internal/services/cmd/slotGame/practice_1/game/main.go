...
func (m *Main) Run(settingsTables *settings.LineTables, isPrintMore bool) {
	log.Info("----------------------MG start----------------------")
	...
	base := table.Base{Board: board, IsPrintMore: isPrintMore, MinimumCount: m.GameParam.PayOutMinimumCount.Mg}
	score := base.CalculateWinScore(m.GameParam.SingleBet, settingsTables.Pay, settingsTables.Line)
